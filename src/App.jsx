...
export default function App() {
  ...
  const [sideNavScrollPosition, setSideNavScrollPosition] = useState(
  ...
  const [activeAdminOption, setAdminOption] = useState(
    localStorage.getItem("activeAdminOpt")
  );
  ...
  const updateAlertContent = (content) => {
    setAlertContent((prevState) => [
      ...prevState,
      { id: prevState.length, content: content },
    ]);
  };
  ...
  const setChatData = (user, text) => {
    setChatBoxData((prevData) => [...prevData, { User: user, Message: text }]);
  };

  // Toggle transfer ticket modal
  const toggleTransferTicket = () =>
    setTransferTicketOpen(!istransferTicketOpen);
  ...
  const setCartItems = (cartItem, action) => {
    if (action === "Add") {
      setUserCart((prevState) => [...prevState, cartItem]);
    } else {
