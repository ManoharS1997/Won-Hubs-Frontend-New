...
const FeedbackUserView = () => {
    ...
    useEffect(() => {
        ...
        setFormValues(prev => ({ ...prev, dateOfSubmission: formattedDate }));
    }, []);
    ...
    const OnChangeInResponse = (dataType, sectionID, contentID, e, dataItemID) => {
        setFormValues(prevValues => {
                            ...
                            if (content.id === contentID) {
                                switch (dataType) {
                                    ...
                                    case "Multiple Choice": {
                                        ...
                                        const selectedChoice = updatedData.find(i => i.isItemChecked);
                                        responseValue = selectedChoice ? selectedChoice.value : "";
                                        return { ...content, type: { ...content.type, data: updatedData }, response: responseValue };
                                    }
                                    case "CheckBox": {
                                        const updatedData = content.type.data.map(i =>
                                            i.itemID === dataItemID
                                        ...
                                        responseValue = updatedData.filter(i => i.isItemChecked).map(i => i.value);
                                        return { ...content, type: { ...content.type, data: updatedData }, response: responseValue };
                                    }
                                    case "Dropdown": {
                                        responseValue = e.target.value;
            ...
            let userResponse = prevValues.responses.find(resp => resp.userId === userId);
            ...
            const existingAnswer = userResponse.answers.find(ans => ans.sectionId === sectionID && ans.questionId === contentID);
            if (existingAnswer) {
                existingAnswer.answer = responseValue;
            } else {
                userResponse.answers.push({
                    sectionId: sectionID,
                    sectionTitle: updatedSections.find(sec => sec.id === sectionID).sectionTitle, // Include section title
                    questionId: contentID,
                    question: updatedSections.find(sec => sec.id === sectionID).content.find(cont => cont.id === contentID).question, // Include question text
            ...
            return {
                ...
                responses: [...prevValues.responses],
            };
        });
    };
    ...
    return (
        <MainContainer>
            ...
            <>
                ...
                {ispopTextVisible && (
                    <SubmitPopUpText
                        style={{ background: submitPopText.includes("Error") ? "red" : "#29bf12" }}
                    >
                        {submitPopText}
                    </SubmitPopUpText>
                )}
                <SubContainer style={{ position: "relative" }}>
                    <CustomContainer>
                        <TemplateImgContainer
                            style={{ background: formValues.image ? `url(${formValues.image})` : "#fff" }}
                        ></TemplateImgContainer>

                        {formValues.sections.map((item, index) => (
                            <div key={item.id} style={{ marginTop: "15px", height: "fit-content" }}>
                                {formValues.sections.length > 1 && (
                                    <SectionTag>
                                        Section {index + 1}/{formValues.sections.length}
                                    </SectionTag>
                                )}
                                <TitleContainer key={item.id} style={{ marginTop: formValues.sections.length <= 1 ? "10px" : "5px", height: "180px" }}>
                                ...
                                <FieldsList>
                                    {item.content.map(each => (
                                        <FieldContainer key={each.id}>
                                            <QuestionDiv
                                                style={{ background: "#f5f3f4", marginBottom: "5px", borderRadius: "10px", position: "relative" }}
                                            >
                                                <CustomTextField
                                            ...
                                            <ResponseDiv style={{ width: "100%" }}>
                                                {each.type.datatype === "Short Text" && (
                                                    <CustomTextField
                                                        ...
                                                        onChange={e => OnChangeInResponse("Short Text", item.id, each.id, e, "")}
                                                ...
                                                {each.type.datatype === "Paragraph" && (
                                                    <CustomTextArea
                                                        ...
                                                        onChange={e => OnChangeInResponse("Paragraph", item.id, each.id, e, "")}
                                                    />
                                                )}
                                                        ...
                                                        {each.type.data.map(box => (
                                                            <RadioBtnContainer key={box.itemID}>
                                                                <input
                                                                    ...
                                                                    onChange={e => OnChangeInResponse("Multiple Choice", item.id, each.id, e, box.itemID)}
                                                                />
                                                                <input
                                                                    readOnly
                                                        ...
                                                        {each.type.data.map(eachBox => (
                                                            <RadioBtnContainer key={eachBox.itemID}>
                                                                <input
                                                                    ...
                                                                    onChange={e => OnChangeInResponse("CheckBox", item.id, each.id, e, eachBox.itemID)}
                                                                />
                                                                <input
                                                                    readOnly
                                                                    type="text"
                                                ...
                                                {each.type.datatype === "Dropdown" && (
                                                    <div>
                                                        <select
                                                            style={{ width: "150px" }}
                                                            value={each.response}
                                                            onChange={e => OnChangeInResponse("Dropdown", item.id, each.id, e, "")}
                                                        >
