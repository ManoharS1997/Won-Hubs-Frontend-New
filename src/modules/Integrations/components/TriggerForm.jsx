...
import { eventsData } from "../../../shared/Data/connectionFormsData";
...
const TriggerForm = ({ appsData, connectionsSources, formData, setFormData, nodeData, setNodes, changeEvent }) => {
  ...
  useEffect(() => {
    formData.app && setTriggerEvents(eventsData[formData.app])
  ...
  useEffect(() => {
    if (formData.event !== '' && formData.app !== '' && eventsData[formData.app][formData.event]) {
      if (formData.event && !nodeData.data.formData.configure && !nodeData.data.formData.test || changeEvent !== formData.event) {
        setFormData(prev => {
          return {
            ...prev,
            configure: eventsData[formData.app][formData.event]?.configure,
            test: eventsData[formData.app][formData.event]?.test
          }
        })
        setCompletedSteps(nodeData.data.setFormData?.completedSteps ? nodeData.data.setFormData.completedSteps : [])
        setCurrentStep(nodeData.data.setFormData?.currentStep ? nodeData.data.setFormData.currentStep : 0)
        setNodes(prev => {
  ...
  const validateTest = async () => {
    ...
    if (formData.test) {
      ...
      // await testSlackLatestMessages()
      // await testSlackQueryMatchedMessages()
    }
  ...
  }, [nodeData.id, setFormData, setNodes]);
  ...
  const onChangeKeyValue = (newPairs, name, section = "configure") => {
    setFormData((prev) => ({
      ...prev,
      [section]: {
        ...prev[section],
              ...
              data: {
                ...
                formData: {
                  ...
                  [section]: {
                    ...node.data.formData[section],
                    [name]: {
                      ...node.data.formData[section][name],
                      value: newPairs,
                    },
                  },
                },
  ...
  const handleStepClick = (stepIndex) => {
    if (completedSteps.includes(stepIndex) || stepIndex === currentStep) {
      ...
      // Clear any existing errors when switching steps
      setErrors({});
    }
  };
    ...
    switch (field.type) {
      case 'input':
        return <CustomInput
          label={label}
          type={field.type}
          placeholder={` ${field.placeholder ? field.placeholder : 'Enter ' + label}`}
          value={field.value}
  ...
  return (
    <div className="h-[95%] flex flex-col overflow-auto">
      ...
      <form
        ...
        className="w-full h-[95%] overflow-auto flex flex-col gap-4 px-2 pb-4"
      >
        {currentStep === 0 && (
          <>
            <CustomDropdown
            ...
            <CustomDropdown
              ...
              options={
                triggerEvents
                  ? Object.keys(triggerEvents).map((event) => ({
                    label: event,
                    option: event,
                  }))
                  : []
        ...
        {currentStep === 1 && (
          <>
            {Object.keys(formData.configure).map((field) =>
              renderField(formData.configure[field], field, "configure")
            )}
          </>
        )}
        ...
        {currentStep === 2 && (
          <>
            {formData?.test &&
              Object.keys(formData.test).map((field) =>
                renderField(formData.test[field], field, "test")
              )}
            {formData?.test?.testResult?.data &&
              Array.isArray(formData?.test?.testResult?.data) ? (
              <ul className="p-0 flex flex-col gap-2">
                <p className=" font-semibold px-2">
                  Pulled {formData?.test?.testResult?.data.length} Records.
                </p>
                  ...
                  return (
                    <div key={index} className=" p-2 flex flex-col gap-2">
                      ...
                      >
                        ...
                        <IoIosArrowDown
                          className={`${expandedItems.includes(index)
                      ...
                      <div
                        className={`flex flex-col gap-2 ml-4 border-l-2 border-gray-300 pl-2 ${expandedItems.includes(index) ? "block" : "hidden"
                          }`}
                      >
                        {Object.keys(item).map((keyItem) =>
                          renderCollapsibleContent(keyItem, item[keyItem])
                        )}
                      </div>
                    </div>
                  );
              ...
              typeof formData?.test?.testResult?.data === "object" ? (
              <div className="p-2 flex flex-col gap-2">
                {Object.keys(formData?.test?.testResult?.data).map(
                  (keyItem) =>
                    renderCollapsibleContent(
                      keyItem,
                      formData?.test?.testResult?.data[keyItem]
                    )
                )}
              </div>
            ) : (
              <div className="w-full h-full flex items-center justify-center">
                {!isTesting ? (
                  ...
                  <div
                    className="absolute w-12 h-12 bg-[#ffffffb9] rounded-full animate-[skLinRotate_1s_ease-in-out_infinite_alternate] 
        ...
        <button
          ...
          className="self-end mt-auto !bg-[var(--background-color)] text-[var(--text-color)] font-bold px-3 py-2 rounded-full flex items-center gap-2 border !border-gray-200"
        >
...
TriggerForm.propTypes = {
  ...
  connectionsSources: PropTypes.array.isRequired,
  webhooksList: PropTypes.array.isRequired,
};
