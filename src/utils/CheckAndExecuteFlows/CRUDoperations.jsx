...
export const PasswordChangeVerify = async (password) => {
  const response = await fetch(`${apiUrl}/api/admin/login/verify/password-change`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      "Authorization": `Bearer ${Cookies.get('accessToken')}`
...
export const VerifyPasswordUpdateEmailOTP = async (otp, new_password) => {
  try {
    const response = await fetch(`${apiUrl}/api/admin/login/verify/password-update-otp`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        "Authorization": `Bearer ${Cookies.get('accessToken')}`
...
export const CreateNewAPIKey = async (apiKeyName, description, userId) => {
  try {
    ...
    const options = {
      ...
      body: JSON.stringify({ name: apiKeyName, description: description, userId })
    }
...
export const GetActiveAPIKeys = async () => {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Error getting active API keys. Status: ${response.status}`);
    }
    const data = await response.json()
...
export const GetWebhookById = async (webhookId) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Error getting webhook by ID. Status: ${response.status}`);
    }
    const data = await response.json()
...
export const GetExternalRegisterFields = async () => {
  try {
    const url = `${import.meta.env.VITE_HOSTED_API_URL}/api/external/register-form`
    ...
    console.error('Error getting the Externa User registration form fields: ', err)
  }
}

export const GetBecomePartnerFields = async () => {
  try {
    const url = `${import.meta.env.VITE_HOSTED_API_URL}/api/get/become-parter-form`
    const results = await axios.get(url,
      {
        method: 'GET',
        headers: {
...
export const GetdesignDepartmentData = async () => {
  try {
    const url = `${import.meta.env.VITE_HOSTED_API_URL}/api/get/design-department-data`
    const results = await axios.get(url,
      {
        method: 'GET',
        headers: {
...
export const GetS3SignedUrl = async (fileName, fileType) => {
  try {
    ...
    return { success: false, error: 'Error generating presigned url in S3 bucket.' }
...
export const createNewRecordInTable = async (recordData, tableName, componentName, pageUrl) => {
...
export const getRecordData = async (tableName, recordId, componentName, pageUrl) => {
  let eventData = {
    time: time,
    localIp: localIp,
...
export const updateTableData = async (tableName, id, recordData, componentName, pageUrl) => {
  // console.log(recordData)
  try {
    let eventData = {
      time: time,
...
export const DeleteRecord = async (tableName, recordId, componentName, pageUrl) => {
  let eventData = {
    time: time,
    localIp: localIp,
...
export const TestZendeskApi = async (url, username, secretKey) => {
  try {
    ...
    const options = {
      ...
      body: JSON.stringify({ url: url, username: username, password: secretKey })
    }
    const response = await fetch(URL, options)
    const data = await response.json();
...
export const sendEmailOtp = async (email, componentName, pageUrl, eventType) => {// console.log(recordData)
  let eventData = {
    time: time,
    localIp: localIp,
...
export const CreateExternalUser = async (formData) => {
  try {
    ...
    const results = await axios.post(`${apiUrl}/api/users/create/external/user`, payload)
    console.log(results)
    return results?.data
  } catch (err) {
...
export const createNewAlert = async (title, type, shortDescription, createdBy, created) => {
  const alertDetails = {
    title,
    type,
    shortDescription,
