...
} from "../APIFetches/APIFetches";
import { useQuery } from "@tanstack/react-query";
import _ from "lodash";

import {
  createNewTicket,
  createNewRecordInTable,
  deleteTicket,
  getTableData,
  updateTableData,
  sendEmail,
...
export default function ExecuteFlows(flowType, record) {
    ...
    const switchType = () => {
      const mapFlows = () =>
        activeFlows.map(async (flow) => {
          const isFlowExisted = record.flows_stage.find(
            (item) => item.id === flow.id
          );
          !isFlowExisted
            ? await updateTableData("ticket", record.id, {
                flows_stage: [
                  ...record.flows_stage,
                  {
                    id: flow.id,
                    nodes: flow.data.activeNodes,
                    edges: flow.data.activeEdges,
                  },
                ],
              })
            : null;
          DivergeNodeAction(
            flow.data.activeNodes,
            flow.data.activeEdges,
            emailFlowMailId,
            flow.data.activeNodes,
            flow
          );
  ...
  const checkForNewRecords = async (tableName) => {
    ...
    if (!tableData.error) {
      const newRecords =
        tableData[tableName].filter((item) => item.label === "new") || [];
    ...
    switch (operation) {
      case "Contains":
        return tableData[node.data.table].some((item) =>
          item.id === record.id
            ? typeof item[filter.name] === "object" &&
              item[filter.name] !== null
              ? compareObject(item)
              : item[filter.name] !== null
              ? item[filter.name].toLowerCase()?.includes(value.toLowerCase())
              : false
            : false
        );
      ...
      case "Is":
        ...
        return tableData[node.data.table].some((item) => {
          if (item.id === record.id) {
            ...
            return (
              item[filter.name]?.toString().toLowerCase() ===
              value.toLowerCase()
            );
          }
        });
      ...
      case "Is Not":
        return tableData.every(
          (item) => item[filter.name]?.toString() !== value
        );
      ...
      case "Is One Of":
        () => {
          ...
          return tableData.some((item) =>
            values.includes(item[filter.name]?.toString())
          );
        };
        break;

      case "Is Empty String":
        return tableData[node.data.table].some(
          (item) => item[filter.name] === ""
        );

      case "Less Than Or Is":
        return tableData.some(
          (item) => parseFloat(item[filter.name]) <= parseFloat(value)
        );
      ...
      case "Is Same":
        return tableData.some(
          (item) =>
            item[filter.name]?.toString() === node[filter.name]?.toString()
        );
      ...
      case "Before":
        return tableData.some(
          (item) => new Date(item[filter.name]) < new Date(value)
        );
  ...
  const sendEmailNotification = (notificationData) => {
    ...
    return `Email notification sent with data: ${JSON.stringify(
      notificationData
    )}`;
  };
  ...
  const sendSmsNotification = (notificationData) => {
    ...
    return `SMS notification sent with data: ${JSON.stringify(
      notificationData
    )}`;
  };
  ...
  const sendMail = async (nodes, node, allNodes, flow) => {
    ...
    const htmlOutput = node.data.mailContent.content
      ? convertToHTML(JSON.parse(node.data.mailContent.content))
      : null;
    try {
      const mailAddress = record.requested_email
      ...
      const mailOptions = {
        ...
        to:
          node.data.customLabel.split(" ").includes("Creation") ||
          record.from_address ||
          record.requested_email
            ? mailAddress
            : "kartheek.muppiri@nowitservices.com",
      ...
      const updatedFlowStage =
        record.flows_stage.length > 0
          ? record.flows_stage.map((item) => {
              if (item.id === flow.id) {
                return {
                  id: item.id,
                  nodes: item.nodes.map((nodeItem) =>
                    nodeItem.id === node.id
                      ? { ...nodeItem, stage: nodeIndex, completed: true }
                      : nodeItem
                  ),
                };
              }
          ...
          : [
              {
                ...
                nodes: nodes.map((nodeItem) =>
                  nodeItem.id === node.id
                    ? { id: nodeItem.id, stage: nodeIndex }
                    : nodeItem
                ),
              },
            ];
      ...
      await updateTableData("ticket", record.id, {
        state: "in_progress",
        flows_stage: JSON.stringify(updatedFlowStage),
      });
      // }
  ...
  const ExecuteDTFUlogic = async (node) => {
    ...
    while (!conditionMet) {
      conditionMet = tableData.some((item) =>
        checkCondition(node.data.condition, [item], node)
      );
      if (!conditionMet) {
  ...
  const configureNodeExecution = async (node, allNodes, nodes, flow) => {
    ...
    const previousNode =
      record.flows_stage
        .filter((item) => item.id === flow.id)[0]
        ?.nodes.slice(nodeIndex - 1, nodeIndex)[0] || null;

    const previousNodeStatus = previousNode.completed
      ? previousNode.completed
      : false;

    // console.log(previousNodeStatusCompleted)
    const currentNodeStatus =
      record.flows_stage !== null && record.flows_stage.length > 0
        ? record.flows_stage
            .find((item) => item.id === flow.id)
            .nodes.find((nodeItem) => nodeItem.id === node.id)
        : {};
  ...
  const responseNodeExecution = async (node, allNodes, nodes, flow) => {
    ...
    if (previousNodeStatus && previousNodeStatus === true) {
      if (currentNodeStatus.completed) {
        // if completed key-pair exists
        if (currentNodeStatus.completed === false) {
          // if current nod enot executed
          console.log(node);
          if (previousNode.type === "configure") {
            //  if previous node is a configure node
            try {
              const status = await conditionsExecution(previousNode); //  getting status of the configure node
              if (node.conditionIs && node.conditionIs === status) {
                // comparing with current  response node path
            ...
            // if not a configure node execute the response node
            switch (node.data.coreAction) {
              case "Send Email":
                return sendMail(nodes, node, allNodes, flow);
              case "Create Record":
        ...
        // if completed key-pair doesn't existed
        if (previousNode.type === "configure") {
          // if previouse node is configure
          try {
            const status = await conditionsExecution(previousNode);
            if (node.conditionIs && node.conditionIs === status) {
              switch (node.data.coreAction) {
        ...
        case "trigger":
          switch (node.data.trigger) {
            ...
            case "On Creation/Updation":
              ...
              triggerSatisfied =
                checkForNewRecords(node.data.table) ||
                checkTicketUpdation(node.id, node.data.table);
              break;
            default:
              return null;
          }
